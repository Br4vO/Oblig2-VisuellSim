...
Balls::Balls(float x, float y, float z) : m_x(0.0f), m_y(0.0f), m_z(0.0f), m_r(0.5f), m_retning(-1.0f)
{
    ...
    retning = ballTransform.mPosition;
...
QVector3D Balls::updateBall()
{
    ...
    retning += (QVector3D(ballTransform.mPosition.x(), 0, ballTransform.mPosition.z()) - QVector3D(prevPos.x(), 0, prevPos.z()) + QVector3D(mHeight->mMapData[i].normal.x(), 0, mHeight->mMapData[i].normal.z())) / 7;
    akselerasjon += ((mHeight->mMapData[i].normal + QVector3D(0.0f, gravity, 0.0f)) * gravity * mHeight->mMapData[i].normal.y()) / 7;
